...
- **예약 페이지(/stays/reserve)는 숙박(contentTypeId 32) 전용이다.**
  체험하기에서 결제 경로를 없앴는데, AI 코스 정류장 링크가 종류와 무관하게 전부
  이 페이지를 가리켜서 우회로가 돼 있었다 — 관광지인 북촌한옥마을에도 숙박 요금표와
  "94,000원 결제하고 예약 확정하기"가 붙었다(실측 확인).
  - BookingPage가 숙박이 아닌 항목을 받으면 상세페이지로 돌려보낸다.
  - 정류장 링크는 `src/utils/stopLink.js`가 종류로 가른다(숙박=예약, 그 외=상세).
    종류를 모르면(예전에 저장돼 contentTypeId가 없는 코스) 상세로 보낸다.
  - 이를 위해 AI 후보와 서버 응답이 contentTypeId를 함께 나른다.
- **마이페이지 "다가오는 여정"은 살아 있다.** 예약을 만드는 경로가 머무르기에 남아 있다
  (숙소 카드 → /stays/reserve → saveReservation). 체험 예약만 없앤 것이라 빈 기능이 아니다.
  다만 요금·정원·환불규정은 여전히 booking.js의 데모 값이다(TourAPI에 없는 값).
- **결제 플로우를 걷어냈다. 지어낸 값이 남아 있는 화면이 없다.**
  예약 페이지는 요금·할인 바우처·결제수단·환불규정이 전부 booking.js의 데모 값이었고,
  장소와 무관하게 "북촌 무형문화재 명인 자개 소반 만들기"가 기본값이라 **숙소를
  예약해도 그 문구가 화면과 마이페이지까지 따라 들어갔다**(사용자 제보로 확인).
  TourAPI는 예약·요금을 제공하지 않으므로 사실로 채울 방법이 없다.
  - 지금은 "내 여정에 담기"다. 장소 정보는 TourAPI 실데이터, 사용자가 고르는 값은
    방문 예정일·인원·메모뿐이다. 금액은 화면에도 저장 데이터에도 없다.
  - contentId 없이 들어오면 대표 장소를 끼워 넣지 않고 오류를 보여준다.
    고르지도 않은 곳이 대상으로 뜨는 게 더 나빴다.
  - `src/data/booking.js`는 삭제했다. 저장 스키마에서 totalLabel·slotLabel을 빼고
    note를 넣었다.
  - 체험하기의 신뢰 문구 3개도 근거가 없어 바꿨다("문화재청 검증", "3일 전 100%
