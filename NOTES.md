...
- 데이터는 전부 목업(`src/data/mypage.js`, `src/data/experiences.js`, `src/data/stays.js`, `src/data/booking.js`). 버튼 클릭 동작은 대부분 없음 — 예외: 스테이 카드 하트 찜 토글, 예약 페이지의 달력/시간대 선택·인원 카운터·결제수단 선택(총액 실시간 계산)·환불규정 아코디언은 실제로 동작함.
